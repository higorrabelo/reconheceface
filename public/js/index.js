...
const loadLabels = () => {
    const labels = ['Higor','Yshrael','Felipe']
    return Promise.all(labels.map(async label => {
        const descriptions = []
        for (let i = 1; i <= 2; i++) {
            const img = await faceapi.fetchImage(`./public/lib/imagens/${label}/${i}.jpg`)
...
const modelos = "./public/lib/models";
    ...
    setInterval(async () => {
        ...
        resizedDetections.forEach(detection => {
            ...
            new faceapi.draw.DrawTextField([
                ...
                ` Sexo ${gender=='male'?'Masculino':'Feminino'} (${parseInt(genderProbability * 100, 10)})`
